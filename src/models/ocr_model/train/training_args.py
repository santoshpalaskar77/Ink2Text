CONFIG = {
    ...
    "output_dir": "/scratch/santosh/large_data/results/sample_test",  #"./scratch/santosh/",     #"train_result"     # Output directory
    "overwrite_output_dir": False,         # If the output directory exists, do not delete its content
    "report_to": ["tensorboard"],          # Repordt logs to TensorBoard
    ...
    "save_total_limit": 2,                 # Maximum number of models to save. The oldest models will be deleted if this number is exceeded
    ...
    "gradient_accumulation_steps": 2,      # Gradient accumulation steps, consider this parameter to achieve large batch size effects when batch size cannot be large
    ...
    "dataloader_num_workers": 4,           # Default is not to use multiprocessing for data loading, usually set to 4*number of GPUs used
